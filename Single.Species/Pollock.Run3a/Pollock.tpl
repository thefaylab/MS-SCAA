//SpZ, Run 33: 
// Based on Run 31 (age-structured obj fx); Adding additional variables to report file
...
PARAMETER_SECTION
  ...
  init_bounded_matrix dAge1(1,nsp,fyr+1,lyr,-5,5,3);   //Annual deviations in recruits, log space
...
FUNCTION calc_negative_loglikelihood
  ...
  */
  ...
  ofv = sum(ofvsp) + sum(tBpen) + sum(tYpen) + sum(tRpen) + sum(Devs);
  ofv_ideal = sum(ofvsp_ideal) + sum(tBpen) + sum(tYpen) + sum(tRpen) + sum(Devs);
...
REPORT_SECTION
  ...
  for (i=1; i<=nsp; i++)
    ...
    report<<"obs.CAA."<<i<<endl<<C(i)<<endl;
    ...
    for (j = 1; j<= nFIC; j++)
      {
      report<<"obs.FIC."<<j<<".sp."<<i<<endl<<FIC(i)(j)<<endl;
